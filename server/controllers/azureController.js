...
const processVideo = async (userId, questionId) => {
    const videoBlobName = `${userId}/${questionId}.webm`;
    const videoFilePath = path.join(__dirname, 'video.webm');
    const audioFilePath = path.join(__dirname, 'audio.wav');
    const transcriptBlobName = `${userId}/${questionId}.txt`;
    ...
    try {
        ...
        const transcriptSasUrl = await generateSasTokenForBlob(userId, transcriptBlobName);
...
const handleTranscription = async (req, res) => {
    ...
    try{
        await processVideo(userId, questionId);
...
const generateSasToken = async (req, res) => {
    const { userId, questionId } = req.params;
    try {
        const sasUrl = await generateSasTokenForBlob(userId, `${userId}/${questionId}.webm`);
        res.json({ sasUrl });
    } catch (error) {
