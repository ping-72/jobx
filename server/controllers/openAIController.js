...
const getTokenCount = (messages, model) => {
    ...
    return messages.reduce((acc, message) => acc + enc.encode(message.content).length, 0);
};
...
const calculatePrice = (model, inputTokens, outputTokens) => {
    ...
    switch (model) {
      ...
      case 'gpt-3.5-turbo-instruct':
        pricePerMillionInputTokens = 1.50;
        pricePerMillionOutputTokens = 2.00;
        break;
      case 'gpt-4o':
    ...
    const priceOutputTokens = (outputTokens / 1000000) * pricePerMillionOutputTokens;
...
const evaluateTranscription = async (req, res) => {
    ...
    try {
      ...
      const { priceInputTokens, priceOutputTokens, totalPrice } = calculatePrice(model, inputTokens, outputTokens);
  
      console.log("response: ", response);
      console.log(`Input Tokens: ${inputTokens},\n Output Tokens: ${outputTokens}`);
      console.log(`Price of Input Tokens: $${priceInputTokens.toFixed(6)},\n Price of Output Tokens: $${priceOutputTokens.toFixed(6)},\n Total Price: $${totalPrice.toFixed(6)}`);
      ...
      res.status(500).json({ error: 'Error evaluating interview', details: error.message });
    }
  };
